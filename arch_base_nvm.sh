...
mkfs.fat -F32 /dev/${droot}p1

#parted -sa opt /dev/${droot} mkpart primary ext4 0% 100%
parted -sa opt /dev/${droot} mkpart primary ext4 512MiB 100%
mkfs.ext4 -F -L ${label_name} /dev/${droot}p2

mount /dev/${droot}p2 /mnt
mkdir /mnt/boot
mount /dev/${droot}p1 /mnt/boot
...
pacman -Sy  base-devel linux-lts openssh vim git wget curl unzip zsh vi wpa_supplicant --noconfirm
